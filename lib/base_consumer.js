...
BaseConsumer.prototype._fetch = function () {
    var self = this;
    self.client.debug('Call _fetch');

    return Promise.try(function () {
        self.client.debug('Call Promise.try');
        ...
        return self.client.fetchRequest(data).map(function (p) {
            ...
            self.client.debug('Empty fetch', p.topic + ':' + p.partition);
            return null;
    ...
    .then(function () {
        self.client.trace('Scheduling next fetch in ' + self.options.idleTimeout + 'ms; ' + JSON.stringify(self.subscriptions));
...
BaseConsumer.prototype._updateSubscription = function (topic, partition) {
    ...
    self.client.debug('Updating subscription', topic + ':' + partition);
    return self.client.updateMetadata().then(function () {
        self.client.debug('Updated subscription', topic + ':' + partition);
        var s = self.subscriptions[topic + ':' + partition];
...
BaseConsumer.prototype.unsubscribe = function (topic, partitions) {
    ...
    return Promise.map(partitions, function (partition) {
        self.client.trace('Unsubscribe from', topic + ':' + partition);
...
BaseConsumer.prototype.end = function () {
    ...
    self.client.trace('End consumer');
