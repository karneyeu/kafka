...
Connection.prototype.connect = function (logger) {
    ...
    if (self.connected) {
        if (self.logger) self.logger.debug('Connection:  connect->connected');
        return Promise.resolve();
    }

    if (self.connecting) {
        if (self.logger) self.logger.debug('Connection:  connect->connecting');
        return self.connecting;
    }
    if (self.logger) self.logger.debug('Connection:  connect->Promise.race');
    self.connecting = Promise.race([
        new Promise(function (resolve, reject) {
            setTimeout(function () {
                if (self.logger) self.logger.debug('Connection:  connect->timeout');
                reject(new NoKafkaConnectionError(self.server(), 'Connection timeout'));
            }, self.options.connectionTimeout);
        }),
        new Promise(function (resolve, reject) {
            var onConnect = function () {
                if (self.logger) self.logger.debug('Connection:  connect->onConnect');
                self.connected = true;
                resolve();
            };
            ...
            self.socket.setTimeout(30000);
            self.socket.on('timeout', function () {
                if (self.logger) self.logger.debug('Connection:  connect->timeout');
                self._disconnect(new NoKafkaConnectionError(self.server(), 'Kafka server timeout'));
            });
            self.socket.on('close', function () {
                if (self.logger) self.logger.debug('Connection:  connect->close');
                self._disconnect(new NoKafkaConnectionError(self.server(), 'Kafka server has closed connection'));
            });
            self.socket.on('end', function () {
                if (self.logger) self.logger.debug('Connection:  connect->end');
                self._disconnect(new NoKafkaConnectionError(self.server(), 'Kafka server has closed connection'));
            });
            self.socket.on('error', function (err) {
                if (self.logger) self.logger.debug('Connection:  connect->error');
                var _err = new NoKafkaConnectionError(self.server(), err.toString());
                reject(_err);
                self._disconnect(_err);
            });
            self.socket.on('data', function(data) {
                 self._receive(data, self.logger);
            });
        })
    ])
    .finally(function () {
...
Connection.prototype._receive = function (data) {
    ...
    var self = this;
    if (self.logger) self.logger.debug('Connection:  _receive');
    if (!this.connected) {
        if (self.logger) self.logger.debug('Connection:  _receive->not connected');
        return;
    }
    ...
    if (self.logger) self.logger.debug('Connection:  _receive correlationId: ' + correlationId);
    ...
    if (data.length > 4 + length) {
        self._receive(data.slice(length + 4));
    }
};
